...
tup = 1, 2.0, 'three'
tup = (1, 2.0, 'three') # same thing, don't need parentheses

# You can access any element with square bracket context
# Counting starts at zero, not 1, indexing
...
#tup[2] = 3 # uncomment this line to try it out; get error
...
# lis1 = 1,2.0,'three' #need square brackets! otherwise this is a tuple
# print(type(lis1))
...
sum1 = 0 	#for every x in p, add x to 0
for x in p:
	sum1 += x

sum2 = 0
for i in range(len(p)): #for every i in the range of the length of p, add the value at index/position i in p
	sum2 += p[i]
...
numbers = [5, 1, 15, 2]
numbers.sort() #sorts the list from lowest to highest
print(numbers)
...
dna = 'AAAGCTAGCTGCTAA'
sep_dna = dna.split('C') #this splits at each C, however you lose each C in the process so be careful
print(sep_dna)
